      ...
      <div v-for="category in mainCategories" :key="category.id" class="col-xs-3 col-sm-4 col-md-2 white-bg">
        <NuxtLink class="element-type" :to="getCategoryLink(category.id, category.name, 1)">
          <div class="element box">
            <p>
              <img class="menu-icon" :src="`/assets/images/menu-icons/${formattedName(category.name)}.png`" />{{
                category.name
              }}
            </p>
          </div>
    ...
    <ul class="dropdown-menu browse-categories mobile-menu" role="menu" aria-labelledby="menu1">
      <li v-for="category in mainCategories" :key="category.id" role="presentation">
        <NuxtLink class="element-type" :to="getCategoryLink(category.id, category.name, 1)">
          <div class="sf-with-ul">
            <p class="category">
              <img class="mobile-menu-icon" :src="`/assets/images/menu-icons/${formattedName(category.name)}.png`" /> {{
                category.name
              }}
            </p>
          </div>
      ...
      <li role="presentation">
        <NuxtLink class="element-type" to="/consultancy-and-design">
          <div class="sf-with-ul">
            <p class="category">
              <img class="mobile-menu-icon" src="/assets/images/menu-icons/consultancy-design.png" /> Consultancy &
              Design
            </p>
          </div>
        </NuxtLink>
...
const fetchMainCategories = async () => {
  try {
    const response = await api.get('/api/get-main-categories/370', {});
